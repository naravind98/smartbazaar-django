...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        if is_cart_item_exists:
            ...
            cart_items = CartItem.objects.filter(product=product, user=current_user)  # collection of cart-item. basically a querryset
            ex_var_list = []
            list_id = []
    ...
    # If the user is not authenticated
        ...
        except Cart.DoesNotExist:                 # If you write Cart.ObjectDoesNotExist......you will get error
...
def cart(request, total_price=0, total_quantity=0, cart_items=None):
    ...
    try:
        ...
        """ If this request has a valid user then we will fetch the cart_items which were added to cart by
         this user.This user could have added to cart and then logged in or the user could have logged in and 
         then added to cart. """
        if request.user.is_authenticated:
            ...
            cart_items = CartItem.objects.filter(cart=cart, is_available=True).order_by('id')  # 'cart' is one of the field of Cart model class
...
def checkout(request, total_price=0, total_quantity=0, cart_items=None):
    """ This function is triggered when we click on the checkout button in the cart page. The
     below code is same as cart() view-function. Only inside the render we are giving a different
     html page"""
    try:
        # total_price, total_quantity, cart_items = 0, 0, None
        if request.user.is_authenticated:
